const tambolaModle = require("../../model/tambolaModel");
...
async function generateTicket(req, res) {
  try {
    ...
    let createTicket = await tambolaModle.create({
      Tambola_tickets: finalTicket,
    });
    if (createTicket) {
      return res.status(201).send({ status: true, ticket: createTicket });
    }
  } catch (error) {
...
module.exports.getDrawSequence = getDrawSequence;
